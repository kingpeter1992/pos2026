...
nouveauTicket(): void {
  ...
  private initialiserVente(): void {
    ...
    .subscribe({
      next: ({ produits, tarifs, regles, stocks }) => {
  ...
  getSelectedTarif(): any | null {
    ...
    if (!id) return null;
    return this.tarifs().find(t => Number(t?.id) === Number(id)) ?? null;
  }
  ...
  ajouterDepuisSelection(): void {
    ...
    const produit = this.produitsTarifies().find(p => Number(p.id) === Number(this.produitSelectionId));
    if (!produit) {
...
private fusionnerProduitsAvecStock(produits: any[], stocks: any[]): any[] {
  ...
  return (produits || []).map((p: any) => {
    ...
    return {
      ...p,
      stock: this.toNumber(stockData.stock),
      pmp: this.toNumber(stockData.pmp) // 🔥 IMPORTANT
    };
  });
}
  ...
  private tarifierProduits(
    ...
    const reglesActivesDuTarif = (regles || []).filter((regle: any) =>
      Number(regle?.tarifVenteId) === Number(tarif.id) &&
      regle?.actif === true
    );
  ...
  private buildProduitSansTarif(produit: any): ProduitTarifie {
    ...
    return {
      ...
      prixVenteOriginal: this.toNumber(produit?.prixVente),
      prixAchat: produit?.prixAchat ?? null,
      pmp: produit?.pmp ?? null,

      stock: this.toNumber(produit?.stock ?? produit?.stockDisponible ?? 0),
...
private resolveBaseTarification(produit: any): number {
  const pmp = this.toNumber(produit?.pmp);
...
payer(): void {
  ...
  this.venteStore.save(payloadVente).subscribe({
    next: (saved: any) => {
      ...
      // recharge aussi le stock / produits tarifés
      this.initialiserVente();
    },
    error: (err) => {
      console.error(err);
